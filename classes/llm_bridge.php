...
/**
 * Calls the configured LLM backend directly, mirroring local_sqlchat's llm_bridge.
 *
    ...
    public function perform_request(string $prompt, string $purpose = 'feedback'): string {
        if ($this->backend === 'local_ai_manager') {
            ...
            if (is_null($responsedata) || !is_array($responsedata) || !array_key_exists('generatedcontent', $responsedata)) {
                throw new \moodle_exception('err_retrievingblurb_checkconfig', $this->plugin);
            }
        ...
        throw new \moodle_exception('err_invalidbackend', 'block_course_recommender');
    }
}
